...
namespace MonsterQuest
{
    ...
    {
        public static int Roll(int numberOfRolls, int diceSides, int fixedBonus = 0, int multiplier = 1, int divider = 1)
        {
            if (Console.verbose)
            {
                Console.Indent();
        ...
        public static string GetRollWithHalfTheDice(string diceNotation)
        {
            ...
            int numberOfRolls = int.Parse(diceNotation.Substring(0, dIndex));

            if (numberOfRolls % 2 == 1) throw new ArgumentException("Dice roll has an odd amount of dice and cannot be halved.");

            return $"{numberOfRolls / 2}d{diceNotation.Substring(dIndex + 1)}";
        }
    }
}
