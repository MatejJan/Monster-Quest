...
namespace MonsterQuest
{
    ...
    {
        public void SimulateCombat(GameState gameState)
        {
            ...
            do
            {
                ...
                foreach (Character character in gameState.party.characters)
                {
                    ...
                    monster.ReactToDamage(greatswordDamageAmount);

                    Console.WriteLine($"{character.displayName} hits the {monster.displayName} for {greatswordDamageAmount} damage. The {monster.displayName} has {monster.hitPoints} HP left.");
