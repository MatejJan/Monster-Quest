...
namespace MonsterQuest
{
    public class DamageAmountTypeDamageAmountAmountAlteration : IDamageAmountAlterationRule, IRulesProvider
    {
        public DamageAmountAlterationValue GetDamageAlteration(DamageAmount damageAmount)
        {
            ...
            DamageType[] vulnerabilities = damageAmount.hit.attack.gameState.GetRuleValues((IDamageTypeRule rule) => rule.GetDamageTypeVulnerabilities(damageAmount)).Resolve();
            DebugHelpers.EndLog();

            DebugHelpers.StartLog("Determining resistances … ");
            DamageType[] resistances = damageAmount.hit.attack.gameState.GetRuleValues((IDamageTypeRule rule) => rule.GetDamageTypeResistances(damageAmount)).Resolve();
            DebugHelpers.EndLog();

            DebugHelpers.StartLog("Determining immunities … ");
            DamageType[] immunities = damageAmount.hit.attack.gameState.GetRuleValues((IDamageTypeRule rule) => rule.GetDamageTypeImmunities(damageAmount)).Resolve();
            DebugHelpers.EndLog();
